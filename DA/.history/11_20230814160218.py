...
def gumbel_softmax(logits: torch.Tensor, tau=1, hard=False, eps=1e-10, dim=-1) -> torch.Tensor:
    gumbels = sample_gumbel(logits.shape)
    y = logits + gumbels.to(device=logits.device)  # add gumbel noise to logits
    y_soft = torch.nn.functional.softmax(y / tau, dim=dim)

    if hard:
        y_hard = torch.zeros_like(y_soft).scatter_(dim, y_soft.argmax(dim, keepdim=True), 1.0)
...
class RNN(nn.Module):
    ...
    def forward(self, x):
        ...
        output = gumbel_softmax(output)  # Replace sigmoid with gumbel_softmax
